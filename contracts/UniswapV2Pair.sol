...
contract UniswapV2Pair is UniswapV2ERC20 {
        ...
        );
        {
            ...
            require(
                balance0Adjusted.mul(balance1Adjusted) >=
                    uint(_reserve0 - fee0).mul(_reserve1 - fee1).mul(1000 ** 2),
                "UniswapV2: K"
            );
        }
        {
            ...
            address treasuryAddress = IUniswapV2Factory(factory)
                .treasuryAddress();
            if (
                IUniswapV2Factory(factory).getPair(_token0, wethAddress) ==
                address(0) ||
                IUniswapV2Factory(factory).getPair(_token1, wethAddress) ==
                address(0) ||
                _token0 == wethAddress ||
                _token1 == wethAddress
            ) {
    ...
    function _swapWETHFee(
        ...
        path[0] = tokenAddress;
        path[1] = wethAddress;
        uint[] memory amountsOut = IUniswapV2Router02(routerAddress)
            .getAmountsOut(fee, path);
        if (amountsOut[amountsOut.length - 1] > 0) {
            uint amountOutMin = amountsOut[amountsOut.length - 1].sub(
                amountsOut[amountsOut.length - 1].mul(10) / 100
            );
