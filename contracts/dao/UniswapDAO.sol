...
contract UniswapDAO is Multisig, IDAO {
    ...
    mapping(uint => mapping(address => bool))
        private daoChangeRequestConfirmations;
    ...
    mapping(uint => mapping(address => bool))
        private feeToChangeRequestConfirmations;
    ...
    mapping(uint => mapping(address => bool))
        private treasuryChangeRequestConfirmations;
    ...
    mapping(uint => mapping(address => bool))
        private routerChangeRequestConfirmations;
     ...
     */
    function getDAOChangeRequest(
        uint id
    ) external view returns (DAOChangeRequest memory) {
     ...
     * @param id the id of change feeTo request
     */
    function getFeeToChangeRequest(
        uint id
    ) external view returns (FeeToChangeRequest memory) {
     ...
     */
    function getTreasuryChangeRequest(
        uint id
    ) external view returns (TreasuryChangeRequest memory) {
     ...
     */
    function getRouterChangeRequest(
        uint id
    ) external view returns (RouterChangeRequest memory) {
     ...
     * @param id the id of change feeTo request
     */
    function isFeeToChangeAvailable(
        uint id
    ) external view override returns (address) {
        require(
            feeToChangeRequests[id].status == RequestStatus.Active,
            "not active"
        );
     ...
     */
    function countGetChangeFeeToAffirmativeVotes(
        uint id
     ...
     * @param id the id of feeTo change request
     */
    function confirmFeeToChangeRequest(
        uint id
    ...
    function newVoteForFeeToChangeRequest(bool voteType, uint id) external {
        ...
        _newVoteFor(
            feeToChangeRequestConfirmations,
            id,
            voteType,
            RequestType.FeeToChange
        );
    }
     ...
     */
    function newFeeToChangeRequest(
        address _address
    ) external onlyVoter(msg.sender) returns (uint) {
        ...
        feeToChangeRequestConfirmations[feeToChangeRequestCounter][
            msg.sender
        ] = true;
        emit NewVoteForRequest(
            RequestType.FeeToChange,
            true,
            msg.sender,
            feeToChangeRequestCounter
        );
        return feeToChangeRequestCounter;
    }
     ...
     * @param id the id of change treasury request
     */
    function isTreasuryChangeAvailable(
        uint id
    ) external view override returns (address) {
        require(
            treasuryChangeRequests[id].status == RequestStatus.Active,
            "not active"
        );
     ...
     * @param id request id to be executed
     */
    function countGetChangeTreasuryAffirmativeVotes(
        uint id
    ) external view returns (uint) {
     ...
     */
    function cancelTreasuryChangeRequest(
        uint id
    ) external onlyVoter(msg.sender) {
        require(
            treasuryChangeRequests[id].status == RequestStatus.Active,
            "not active"
        );
     ...
     * @param id the id of treasury change request
     */
    function confirmTreasuryChangeRequest(
        uint id
    ) external override onlyFactory returns (bool) {
        require(
            treasuryChangeRequests[id].status == RequestStatus.Active,
    ...
    function newVoteForTreasuryChangeRequest(bool voteType, uint id) external {
        ...
        _newVoteFor(
            treasuryChangeRequestConfirmations,
            id,
            voteType,
            RequestType.TreasuryChange
        );
    }
     ...
     */
    function newTreasuryChangeRequest(
        address _address
    ) external onlyVoter(msg.sender) returns (uint) {
        require(_address != address(0), "zero address");
        treasuryChangeRequestCounter = treasuryChangeRequestCounter + 1;

        treasuryChangeRequests[
            treasuryChangeRequestCounter
        ] = TreasuryChangeRequest({
            newTreasuryAddress: _address,
            status: RequestStatus.Active
        });

        treasuryChangeRequestConfirmations[treasuryChangeRequestCounter][
            msg.sender
        ] = true;
        emit NewVoteForRequest(
            RequestType.TreasuryChange,
     ...
     * @param id the id of change router request
     */
    function isRouterChangeAvailable(
        uint id
    ) external view override returns (address) {
        require(
            routerChangeRequests[id].status == RequestStatus.Active,
            "not active"
        );
     ...
     * @param id request id to be executed
     */
    function countGetChangeRouterAffirmativeVotes(
        uint id
    ) external view returns (uint) {
     ...
     * @param id the id of router change request
     */
    function confirmRouterChangeRequest(
        uint id
    ) external override onlyFactory returns (bool) {
        require(
            routerChangeRequests[id].status == RequestStatus.Active,
    ...
    function newVoteForRouterChangeRequest(bool voteType, uint id) external {
        ...
        _newVoteFor(
            routerChangeRequestConfirmations,
            id,
            voteType,
            RequestType.RouterChange
        );
    }
     ...
     */
    function newRouterChangeRequest(
        address _address
    ) external onlyVoter(msg.sender) returns (uint) {
        require(_address != address(0), "zero address");
        uint32 size;
        ...
        routerChangeRequestConfirmations[routerChangeRequestCounter][
            msg.sender
        ] = true;
        emit NewVoteForRequest(
            RequestType.RouterChange,
    ...
    function isDAOChangeAvailable(
        ...
        require(
            daoChangeRequests[id].status == RequestStatus.Active,
            "not active"
        );
     ...
     * @param id request id to be executed
     */
    function countGetChangeDAOAffirmativeVotes(
        uint id
    ) external view returns (uint) {
     ...
     * @param id the id of dao change request
     */
    function confirmDAOChangeRequest(
        uint id
    ) external override onlyFactory returns (bool) {
        require(
            daoChangeRequests[id].status == RequestStatus.Active,
    ...
    function newVoteForDAOChangeRequest(bool voteType, uint id) external {
        ...
        _newVoteFor(
            daoChangeRequestConfirmations,
            id,
            voteType,
            RequestType.DAOChange
        );
    }
     ...
     */
    function newDAOAddressChangeRequest(
        address _address
    ) external onlyVoter(msg.sender) returns (uint) {
        require(_address != address(0), "zero address");
        uint32 size;
        ...
        daoChangeRequestConfirmations[daoChangeRequestCounter][
            msg.sender
        ] = true;
        emit NewVoteForRequest(
            RequestType.DAOChange,
