...
contract Multisig {
     ...
     */
    function getVoterRequest(
        uint id
    ) external view returns (VoterRequest memory) {
    ...
    function removeVoter(
        address oldVoterAddress
    ) private onlyVoter(oldVoterAddress) {
        voters[oldVoterAddress] = false;
        activeVotersCount--;
    }

    function _newVoteFor(
        mapping(uint => mapping(address => bool)) storage confirmations,
        uint id,
        bool voteType,
    ...
    function _countGet(
        ...
        for (uint i = 0; i <= getVotersCounter(); i++) {
            if (
                confirmations[id][getVoterById(i)] &&
    ...
    function _consensus(
        ...
        require(
            _countGet(confirmations, id) * 100 >
                (getActiveVotersCount() * 100) / 2,
            "not enough votes"
        );
    }
    ...
    function newVoteForVoterRequest(
        ...
        require(
            voterRequests[id].candidate != address(0),
            "voter address is zero"
        );
    ...
    function newVoterRequest(
        ...
        emit NewVoteForRequest(
            RequestType.VoterChange,
            true,
            msg.sender,
            voterRequestsCounter
        );
    }
     ...
     */
    function countGetVotersAffirmativeVotes(
        uint id
    ...
    function votersRequestConclusion(uint id) external {
        ...
        emit ChangeVoter(
            voterRequests[id].candidate,
            voterRequests[id].include
        );
